...
export function StokBahanReport({ data }) {
    ...
    const handleDownloadPdf = () => {
        ...
        data.data.forEach(({ nama_bahan, satuan, stok_bahan }) => {
            const rowData = [nama_bahan, satuan, stok_bahan.toLocaleString('id-ID')];
            tableRows.push(rowData);
        });

        doc.autoTable(tableColumn, tableRows, { startY: tableStartY });
        doc.save("laporan_stok_bahan.pdf");
    };
        ...
        <div>
            <div className="mx-7" ref={componentRef}>
                ...
                <table className="w-full min-w-[640px] table-auto border-collapse">
                    ...
                    <tbody>
                        {data.data.map(({ nama_bahan, satuan, stok_bahan }, index) => (
                            <tr key={index}>
                                ...
                                <td className="border border-gray-400 py-2 px-4">
                                    <Typography className="text-xs font-[400] text-blue-gray-600">
                                        {stok_bahan.toLocaleString('id-ID')}
                                    </Typography>
                                </td>
                            </tr>
                        ))}
